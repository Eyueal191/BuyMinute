...
const signUpAction = async ({
 ...
 const logInAction = async ({ request }) => {
  try {
    ...
    const { accessToken, userId, userEmail, message, success } = response.data;

    if (!success || !accessToken || !userId) {
      return {
        success: false,
        message: message || "Invalid login response from server",
      };
    }

    // ✅ Return in the shape expected by your LogIn component
    return {
      success: true,
      userEmail,
      accessToken,
