...
function OrderCard({ item, setRefresh }) {
  ...
  const cancelHandler = async () => {
    if (cancelling) return; // Prevent parallel cancelling
    setCancelling(true);

    try {
      const userId = localStorage.getItem("userId");
      if (!item._id) throw new Error("Item ID not found");
      ...
      toast.error(error.response?.data?.message || error.message || "Failed to cancel item");
    } finally {
      setCancelling(false);
    }
  };

  const paymentBadgeClasses = paymentStatus === "succeeded"
    ? "bg-green-100 text-green-700"
    : paymentStatus === "failed"
    ? "bg-red-100 text-red-700"
    : "bg-yellow-100 text-yellow-700";

  const paymentText = paymentStatus === "succeeded"
    ? "Paid"
    : paymentStatus === "failed"
    ? "Failed"
    : "Pending";
    ...
    <div className="w-[90%] mx-auto flex flex-wrap sm:flex-nowrap items-center gap-6 p-4 border rounded-2xl shadow-md bg-white hover:bg-gray-50 transition-all duration-300 hover:shadow-lg">
      ...
      <div className="flex flex-col items-center sm:items-end gap-2">
        <div className="flex items-center gap-2 text-sm sm:text-base font-semibold">
          ...
          <span className={`px-3 py-1 rounded-full text-xs sm:text-sm font-medium ${paymentBadgeClasses} shadow-sm`}>
            {paymentText}
          </span>
        </div>
        ...
        <button
          ...
          className={`text-sm sm:text-base font-semibold transition-colors duration-200 ${
            cancelling ? "text-red-400 opacity-50 cursor-not-allowed" : "text-red-600 hover:text-red-800"
          }`}
        >
